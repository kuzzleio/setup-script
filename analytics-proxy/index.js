...
try {
  logger = new winston.Logger({
    transports: [new winston.transports.File({ filename: config.log.filename })]
  });
...
proxy.post("/", (request, result) => {
  ...
  let userId = uuid(
    request.body.uid.substring(0, 32),
    request.body.uid.substring(32)
  );
  let sessionAnalytics = ua(config.googleAnalytics.id, userId, {
  ...
  switch (request.body.type) {
    case "pulled-latest-containers":
      ...
      mailer
        ...
        .then(() => {
          console.log(
            `[ℹ] Successfull installation reported via mail (${
        ...
        .catch(err => {
          console.error(
            c.red(
              "[✖] Unable to report successfull installation via mail",
              err.message
            )
          );
        });
...
proxy.listen(config.port, () => {
  console.log(
    c.bold.green(`[✔] Kuzzle Analytics proxy listening on port ${config.port}`)
  );
});
